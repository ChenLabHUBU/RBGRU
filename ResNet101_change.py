...
def Conv_Block(input_layer,filters,strides):
    ...
    n_1 = tf.keras.layers.GlobalAveragePooling2D()(x)
    n_1 = tf.keras.layers.Dense(units=1)(n_1)
    n_2 = tf.keras.layers.GlobalAveragePooling2D()(shortcut)
    n_2 = tf.keras.layers.Dense(units=1)(n_2)
    n_1_2 = tf.keras.layers.Concatenate(axis=-1)([n_1,n_2])
    n_1_2 = tf.keras.layers.Softmax(axis=-1)(n_1_2)
    
    n_1 = tf.keras.layers.Lambda(lambda x: x[:, 0] * 2)(n_1_2)
    n_2 = tf.keras.layers.Lambda(lambda x: x[:, 1] * 2)(n_1_2)

    n_1 = tf.keras.layers.Reshape((1, 1, 1))(n_1)
    n_2 = tf.keras.layers.Reshape((1, 1, 1))(n_2)

    x = tf.keras.layers.Add()([
        tf.keras.layers.Multiply()([x, n_1]),
        tf.keras.layers.Multiply()([shortcut, n_2])
    ])
...
def Identity_Block(input_layer,filters):
    ...
    n_1 = tf.keras.layers.GlobalAveragePooling2D()(x)
    n_1 = tf.keras.layers.Dense(units=1)(n_1)
    n_2 = tf.keras.layers.GlobalAveragePooling2D()(input_layer)
    n_2 = tf.keras.layers.Dense(units=1)(n_2)
    n_1_2 = tf.keras.layers.Concatenate(axis=-1)([n_1,n_2])
    n_1_2 = tf.keras.layers.Softmax(axis=-1)(n_1_2)
    ...
    x = tf.keras.layers.Add()([
        tf.keras.layers.Multiply()([x, n_1]),
        tf.keras.layers.Multiply()([input_layer, n_2])
    ])
...
with open(r"./resnet101_class_names.json", 'w', encoding='utf-8') as f:
...
pd.DataFrame(history.history).to_csv(r'./resnet101.csv', index=False)
